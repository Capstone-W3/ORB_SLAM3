...
pcl::PointCloud<PointCloudMapping::PointT>::Ptr PointCloudMapping::generatePointCloud(KeyFrame *kf, cv::Mat color, cv::Mat depth, int seqNum)
{
    ...
    // Only adds points that are within the threshold
...
{
    // Creates point clouds for every key frame since the last one that was published (would usually just be 1 at a time)
    for (size_t i = lastKeyframeSize; i < N; i++) 
    {
        // The point cloud for a particular key frame i
        PointCloud::Ptr p = generatePointCloud(keyframes[i], colorImgs[i], depthImgs[i], 0);
